...
//   });
// });
// ... other imports (crypto, fs, cloudinary, asyncHandler, AppError, User, sendEmail)
import crypto from 'crypto';
import fs from 'fs/promises';
...
// ... registerUser, loginUser, logoutUser, getLoggedInUserDetails, forgotPassword, resetPassword, changePassword, updateUser functions (no changes)
// ... (Paste all existing user controller functions here)
...
/**
 * @GET_MY_COURSES
 * @ROUTE @GET {{URL}}/api/v1/user/my-courses
 * @ACCESS Private(Logged in users only)
 */
export const getMyCourses = asyncHandler(async (req, res, _next) => {
  // Finding the user using the id from modified req object
  const user = await User.findById(req.user.id).populate({
    path: 'purchasedCourses',
    select: 'title description thumbnail category createdBy price', // Select fields you want to show
  });
