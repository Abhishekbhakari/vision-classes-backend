...
// abhishekbhakari/vision-classes-backend/vision-classes-backend-2abfd55e8e05597a6b97163bf590df4d98519459/controllers/user.controller.js
...
export const updateUser = asyncHandler(async (req, res, next) => {
  ...
  const { fullName } = req.body;
  // --- THIS IS THE FIX ---
  // Get the id from req.user (logged-in user) NOT req.params
  const { id } = req.user;
  // --- END OF FIX ---
 ...
 * @ACCESS Private (Logged in user only)
 * @description Fetches all courses purchased by the logged-in user.
 */
export const getMyPurchasedCourses = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.findById(id).populate({
    ...
    select: '-lectures', // Exclude lectures to keep the payload light
  });
